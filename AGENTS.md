...
- **Backend services** (Rust, built with `make`): `register`, `producer`, `consumer`, `alarm-api`, `alarm-receiver`, `alarm-notifier`
...
- `bruno-collections/home-anthill.yml` — Bruno API collection for testing (covers `api-server`, `admission`, and `alarm-api` endpoints)
...
- The script runs preflight checks for MongoDB, Mosquitto, Redis, GUI, `api-server`, `admission`, `alarm-api`, `register`, and `api-devices`, then asks for confirmation before opening the browser.
- The script defaults to `LOGIN_SERVER=http://localhost:4200`, `API_SERVER=http://localhost:8082`, `ADMISSION_SERVER=http://localhost:8099`, `ALARM_SERVER=http://localhost:8089`, `REGISTER_SERVER=http://localhost:8000`, `API_DEVICES_HOST=localhost`, `API_DEVICES_PORT=50051`, `MOSQUITTO_HOST=localhost`, `MOSQUITTO_PORT=1883`, `REDIS_HOST=localhost`, `REDIS_PORT=6379`, and `MONGODB_URL=mongodb://localhost:27017`; override those environment variables before running if needed.
