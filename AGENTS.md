...
- **Backend services** (Rust, built with `make`): `register`, `producer`, `consumer`, `alarm`, `alarm-receiver`, `alarm-notifier`
...
- `bruno-collections/home-anthill.yml` — Bruno API collection for testing (covers `api-server`, `admission`, and `alarm` endpoints)
...
- The script runs preflight checks for MongoDB, Mosquitto, Redis, GUI, `api-server`, `admission`, `alarm`, `register`, and `api-devices`, then asks for confirmation before opening the browser.
